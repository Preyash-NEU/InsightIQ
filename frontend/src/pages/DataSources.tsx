      ...
      <div className="space-y-6">
                ...
                <div className="absolute right-0 z-20 w-64 mt-2 overflow-hidden border shadow-2xl bg-slate-900/95 backdrop-blur-xl border-slate-700/50 rounded-xl">
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-400">CSV, Excel, JSON, Parquet</p>
                    </div>
                  </button>
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-400">PostgreSQL, MySQL, SQLite</p>
                    </div>
                  </button>
                </div>
              )}
        ...
        {loading ? (
          ...
          <div className="py-20 text-center">
            ...
            <h3 className="mb-2 text-2xl font-bold text-white">No Data Sources Yet</h3>
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
            {filteredDataSources.map((source) => {
              ...
              const statusColor = 
                ...
                source.status === 'syncing' ? 'bg-yellow-500/10 text-yellow-400 border-yellow-500/20' :
                ...
                <div key={source.id} className="relative group">
                  ...
                  <div className="relative p-6 transition-all border bg-slate-900/50 backdrop-blur-xl border-slate-800/50 rounded-2xl hover:border-slate-700/50">
                    ...
                    <div className="flex items-start justify-between mb-4">
                      <div className={`w-14 h-14 bg-gradient-to-br ${colors.gradient} rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform`}>
                        {source.type.startsWith('database') ? (
                          <Database className={`w-7 h-7 ${colors.iconColor}`} />
                        ) : source.type === 'excel' ? (
                          <TableIcon className={`w-7 h-7 ${colors.iconColor}`} />
                        ) : (
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="flex items-center justify-center flex-1 px-4 py-2 text-sm font-medium text-white transition-all rounded-lg bg-gradient-to-r from-cyan-500 to-blue-600 hover:shadow-lg hover:shadow-cyan-500/20"
                      >
                        <Eye className="w-4 h-4 mr-2" />
                        Preview
                      </button>
                      <button
                        onClick={() => navigate(`/analysis?source=${source.id}`)}
                        className="flex items-center justify-center flex-1 px-4 py-2 text-sm font-medium text-white transition-all border rounded-lg bg-slate-800/50 border-slate-700/50 hover:bg-slate-700/50"
                      >
                      ...
                      <button
                        ...
                        className="px-4 py-2 transition-all border rounded-lg border-slate-700/50 text-slate-400 hover:text-red-400 hover:border-red-500/50 hover:bg-red-500/10"
                      >
...
const UploadModal = ({ onClose, onSuccess }: { onClose: () => void; onSuccess: () => void }) => {
  ...
  const handleFileChange = async (selectedFile: File) => {
    ...
    if (selectedFile.name.endsWith('.xlsx') || selectedFile.name.endsWith('.xls')) {
      ...
      try {
        ...
        setError('Could not read Excel file. Make sure it\'s a valid Excel file.');
      } finally {
  ...
  const handleUpload = async () => {
    ...
    try {
      await DataSourceService.uploadFile(file, name || undefined, selectedSheet || undefined);
      onSuccess();
    } catch (err: any) {
...
const PreviewModal = ({ 
  ...
  useEffect(() => {
    const fetchPreview = async () => {
      try {
        setLoading(true);
        const previewData = await DataSourceService.getPreview(dataSource.id, 100);
      ...
      <div className="relative w-full max-w-6xl max-h-[90vh]">
        ...
        <div className="relative bg-slate-900/90 backdrop-blur-2xl border border-slate-700/50 rounded-3xl shadow-2xl flex flex-col max-h-[90vh]">
          <div className="flex items-center justify-between p-6 border-b border-slate-700/50">
            <div>
              <h2 className="text-2xl font-bold text-white">{dataSource.name}</h2>
              <p className="text-sm capitalize text-slate-400">{dataSource.type.replace(/_/g, ' ')} • {dataSource.row_count?.toLocaleString()} rows</p>
            </div>
            <button
              onClick={onClose}
              className="p-2 transition-all rounded-lg hover:bg-slate-800 text-slate-400 hover:text-white"
                ...
                <table className="w-full">
                  ...
                  <tbody>
                    {preview.rows.map((row: any, i: number) => (
                      <tr key={i} className="transition-colors border-b border-slate-800/50 hover:bg-slate-800/30">
                        {preview.columns.map((col: string, j: number) => (
                          <td key={j} className="px-4 py-3 text-sm text-slate-400 whitespace-nowrap">
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-black/50 backdrop-blur-sm">
      <div className="relative w-full max-w-md">
        ...
        <div className="relative p-8 border shadow-2xl bg-slate-900/90 backdrop-blur-2xl border-slate-700/50 rounded-3xl">
          ...
          <h2 className="mb-2 text-2xl font-bold text-center text-white">Delete Data Source?</h2>
          <p className="mb-6 text-center text-slate-400">
            Are you sure you want to delete <span className="font-semibold text-white">"{dataSource.name}"</span>? 
